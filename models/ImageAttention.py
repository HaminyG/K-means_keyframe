...
class ResNet(nn.Module):
    ...
    def convlstm(self,x):
        ...
        model = ConvLSTM(input_dim=C,hidden_dim=[C],kernel_size=(3, 3),num_layers=1,batch_first=True,bias = True,return_all_layers = False).cuda()
        output=model(x)
        output=output[0][0].permute(0,2,1,3,4)
    ...
    def forward(self, x):
        ...
        l1 = self.convlstm(self.layer1(x))
        l2 = self.convlstm(self.layer2(l1))
        l3 = self.layer3(l2)
        l4 = self.layer4(l3)
        g = self.avgpool(l4)
        g = g.view(g.size(0), -1)
...
#     data=torch.rand(1,3,16,128,128).cuda()
#     cnn3d = resnet34(pretrained=False,sample_size=sample_size, sample_duration=sample_duration, num_classes=10).cuda()
#     print(cnn3d(data).size())